...
def get_all_orders(current_user, role):
    result, status = OrderController.get_all_orders()
...
def get_stats(current_user, role):
    try:
        orders_result, status = OrderController.get_all_orders()
        if status != 200:
            return jsonify(orders_result), status
...
def mark_order_paid(current_user, role):
    try:
        ...
        result = MongoService.update_one('orders', {'id': order_id}, {'paid': True, 'paid_date': str(datetime.now().date())})
